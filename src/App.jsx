...
function ActiveBookView({ book, onUpdate, onBack }) {
  ...
  const addMember = () => {
    ...
    setNewMemberName('');
    showToast("隊友已新增"); // 3. Updated Wording
  };

  const removeMember = (id) => {
    // Check if involved in transactions (even deleted ones, to maintain integrity, or just active ones? Let's check all to be safe)
  ...
  const addTransaction = () => {
    ...
    const newTx = {
      ...
      isDeleted: false, // 5. Initialize soft delete status
    };
  ...
  const settlementPlan = useMemo(() => {
    ...
    // 5. Filter out deleted transactions for calculation
    const activeTransactions = book.transactions.filter(t => !t.isDeleted);

    // Ensure all involved IDs exist in balance map
